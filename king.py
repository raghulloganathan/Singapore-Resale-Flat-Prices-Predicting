    ...
    with st.sidebar:
        selected = option_menu("Main Menu", ["Basic Overview", "Data Prediction"],
                           styles={"nav-link": {"font": "sans serif", "font-size": "25px", "text-align": "centre"},
                                   "nav-link-selected": {"font": "sans serif", "background-color": "#red"},
                                   "icon": {"font-size": "20px"}
                                   }
                           )
...
# -------------------------------------------------------------------------------------------------
    ...
    if selected == "Basic Overview":
        st.markdown("<h1 style='text-align: center; font-size: 40px; color: OrangeRed;' ><u> Singapore  Resale Flat Prices Predictor</u> </h1>", unsafe_allow_html=True)
        st.markdown("### :red[Overview :] In this project, the goal is to design and implement a robust machine learning model and deploy it as an intuitive web application. The model will leverage historical data from resale flat transactions in Singapore to predict accurate resale prices. ")
        st.markdown("### :red The primary objective is to learn about ML predictions but it also provides a valuable tool for potential buyers and sellers")
        st.markdown("### :red[Libraries Used :] Streamlit, Pandas, Sklearn - Random Forest Regression")
        

    if selected == "Data Prediction":
        st.markdown("# :orange[Data Prediction]")

        st.sidebar.title('Please provides the required details below:')
        ...
        if st.button('Predict'):
            prediction = rf_regressor.predict(features_df)  
